...
def Equation_Solutio(Inv,emat):  # defining a function finding the solution of system of linear equations
    b = []                       # 3*1 matrix for storing the values of constants
    for i in range(3):           # for loop to iterate between the 3 rows
        a = []                   # temprorary list to store the constant values
        for j in range(3,4):     # for loop to access only the 4 the column in the augmented matrix (constant values) 
            x = emat[i][j]       # storing the value of constant to x
            a.append(x)          # appending x to temprorary list
        print(" ")
        b.append(a)              # appending values form temporary list to constant matrix
    result=[[0 for i in range(1)] for j in range(3)]  # zero matrix with dimension of 3*1
    for i in range(3):           # for loop to itetrate between the rows
        for j in range(1):       # for loop to iterate the columns
            for k in range(3):   # for loop to iterate the rows in a column
                result [i][j] += int(Inv[i][k]*b[k][j]) #calculating the solution 
...
if Mat_Det==0:          # if determinant is zero then we cant find the inverse 
    ...
    Eqn_Sol = Equation_Solutio(Mat_Inv,mat1) # calling the function to find the solution of equation
    print("The solution for the equation is :") # printing the solution from returned value
